...
function toolbar.widget(glasses)
    ...
    for i = 1, #hudObjects do
        ...
        if #hudObjects[i].static == 0 and #hudObjects[i].glasses ~= nil then
            ...
            table.insert(
                hudObjects[i].static,
                ar.rectangle(hudObjects[i].glasses, {x + w + 3, y + 9}, clockWidth + 2, clockheight + 2, borderColor)
            ) --Clock Base
            table.insert(
                hudObjects[i].static,
                ar.rectangle(hudObjects[i].glasses, {x, y - 10}, w + 1, 12, borderColor, 0)
            ) --Hide Armor
            table.insert(hudObjects[i].static, ar.rectangle(hudObjects[i].glasses, {x, y + 3}, 2, h - 3, borderColor)) --Left Border
            table.insert(
                hudObjects[i].static,
                ar.quad(hudObjects[i].glasses, {x + 3, y}, {x, y + 3}, {x + w + 3, y + 3}, {x + w, y}, borderColor)
            ) -- Top Border
            table.insert(
                hudObjects[i].static,
                ar.quad(
                    hudObjects[i].glasses,
                    {x + w, y},
                    {x + w, y + h},
                    {x + w + rightTriangleSide, y + h},
                    {x + w + rightTriangleSide, y + rightTriangleSide},
                    borderColor
                )
            ) --Right triangle
            table.insert(
                hudObjects[i].static,
                ar.rectangle(hudObjects[i].glasses, {x + w - 1, y + 15}, 2, h - 15, borderColor)
            ) --Right Toolbar Edge
            table.insert(
                hudObjects[i].static,
                ar.quad(
            ...
            hudObjects[i].dynamic.xpbar =
                ar.quad(
                hudObjects[i].glasses,
                {x + 2, y + 15},
                {x + 2 + hExpBar, y + 15 + hExpBar},
                {x + w, y + 15 + hExpBar},
                {x + w - hExpBar, y + 15},
                primaryColor,
                0.5
            ...
            hudObjects[i].dynamic.diagonal =
                ar.quad(
                hudObjects[i].glasses,
                {x + w, y + 1},
                {x + w, y + 2},
                {x + w + rightTriangleSide - 1, y + 1 + rightTriangleSide},
                {x + w + rightTriangleSide - 1, y + rightTriangleSide},
                primaryColor
            ) --Diagonal Strip
            hudObjects[i].dynamic.rightstrip =
                ar.rectangle(
                hudObjects[i].glasses,
                {x + w + rightTriangleSide - 2, y + rightTriangleSide},
                1,
                h - rightTriangleSide,
                primaryColor
            ) --Right Strip
            hudObjects[i].dynamic.clockEdge =
                ar.rectangle(hudObjects[i].glasses, {x + w + 4, y + 10}, clockWidth, clockheight, primaryColor) --Clock Edge
            table.insert(
                hudObjects[i].static,
                ar.rectangle(hudObjects[i].glasses, {x + w + 5, y + 11}, clockWidth - 2, clockheight - 2, borderColor)
            ) --Clock Panel
